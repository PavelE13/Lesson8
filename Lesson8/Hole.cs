...
namespace Lesson8
{
    ...
    public class Hole : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            Document arDoc = commandData.Application.ActiveUIDocument.Document;
            Document ovDoc = arDoc.Application.Documents.OfType<Document>().Where(x => x.Title.Contains("home_OV")).FirstOrDefault();
            if (ovDoc == null)
            {
            ...
            Transaction hole0 = new Transaction(arDoc);
            hole0.Start("Расстановка отверстий");
            if (!familySymbol.IsActive)
                familySymbol.Activate();
            hole0.Commit();

            Transaction hole1 = new Transaction(arDoc);
            hole1.Start("Расстановка отверстий");
            foreach (Duct duct in ducts)
            {
                ...
                foreach (ReferenceWithContext refer in intersections)
                {
                    ...
                    width.Set(duct.Diameter + 0.164041994750655 * 2);
                    height.Set(duct.Diameter + 0.164041994750655 * 2);
                }
            }
            ...
            foreach (Pipe pipe in pipes)
            {
                ...
                foreach (ReferenceWithContext refer in intersections)
                {
                    ...
                    Parameter width = hole.LookupParameter("Ширина");
                    Parameter height = hole.LookupParameter("Высота");
                    width.Set(pipe.Diameter + 0.164042 * 2);
                    height.Set(pipe.Diameter + 0.082021 * 2);
                }
            }
